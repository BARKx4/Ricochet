...
- Webview words build escaped GUI document fragments and state/action documents
  for `rco gui` and `rco package --gui`; GUI launchers open through the
  platform system browser to avoid the vulnerable GTK/WebKitGTK Rust binding
  stack.
...
- `rco package` creates standalone launcher executables by embedding bytecode
  or MVC bundles.
- `rco package --tui`, `--gui`, and `--gui --mvc` cover console, WebView or
  system-browser GUI launchers, and local-server desktop apps.
