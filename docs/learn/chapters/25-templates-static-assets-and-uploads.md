...
```powershell
cargo run -q -p ricochet_cli --bin rco -- doctor examples/learn/23-mvc/templates_uploads
```
...
```powershell
cargo run -q -p ricochet_cli --bin rco -- serve examples/learn/23-mvc/templates_uploads --host 127.0.0.1 --port 3000
```
...
The checked-in validation path does not start a server or store uploaded files.
