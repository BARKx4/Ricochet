...
- Desktop GUI beta: trusted local scripts can build escaped `webview` document
  maps, preview them with `rco gui`, and package them as system-browser GUI
  launchers with `rco package --gui`. MVC projects can also be packaged as
  local-server desktop GUI apps with `rco package --gui --mvc`.
